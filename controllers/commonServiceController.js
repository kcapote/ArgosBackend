        ...
        .exec(
            (err, commonServices) => {
                if (err) {
                    ...
                    CommonService.countDocuments({}, (err, totalRecords) => {
...
router.get('/all', [authentication.verifyToken, authentication.refreshToken], (req, res, next) => {
    ...
    CommonService.find({ 'recordActive': true })
        .populate('project')
        .sort({ number: 1 })
        .exec(
            (err, commonServices) => {
                if (err) {
                    ...
                    CommonService.find({ 'recordActive': true }).countDocuments({}, (err, totalRecords) => {
                        res.status(200).write(JSON.stringify({
                            success: true,
                            commonServices: commonServices,
                            totalRecords: totalRecords,
        ...
        .exec(
            (err, commonServices) => {
                if (err) {
                    ...
                    CommonService.find({ 'recordActive': recordActive }).countDocuments({}, (err, totalRecords) => {
                        res.status(200).write(JSON.stringify({
                            success: true,
                            commonServices: commonServices,
                            totalRecords: totalRecords,
        ...
        .exec(
            (err, commonServices) => {
                if (err) {
                    ...
                    CommonService.find({ 'project': id, 'type': type, 'recordActive': true }).countDocuments({}, (err, totalRecords) => {
                        res.status(200).write(JSON.stringify({
                            success: true,
                            commonServices: commonServices,
                            totalRecords: totalRecords,
